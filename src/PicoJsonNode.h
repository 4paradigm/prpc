#ifndef PARADIGM4_PICO_COMMON_PICO_JSON_NODE_H
#define PARADIGM4_PICO_COMMON_PICO_JSON_NODE_H
...
@see https://github.com/paradigm4::pico
@since version 1.0.0
*/
namespace paradigm4 {
namespace pico {
...
@sa https://github.com/paradigm4::pico/json/issues/51#issuecomment-86869315
@since version 2.0.0
*/
...
class basic_json
{
  ...
  public:
    ...
    */
    ...
    {
        basic_json dummy(other);
        // check that passed value is valid
        dummy.assert_invariant();

        using std::swap;
        swap(m_type, dummy.m_type);
        swap(m_value, dummy.m_value);
  ...
  public:
    ...
    bool save(FILE* file, bool pretty = true) const {
        if (file == nullptr) {
            LOG(WARNING) << "open file error, FILE* is nullptr";
            return false;
        }
        string_t out_str;
        if (!save(out_str, pretty)) {
            LOG(WARNING) << "write json to FILE* failed: call dump failed";
            return false;
        }
        
        if (fwrite(out_str.c_str(), sizeof(char), out_str.size(), file) != out_str.size()) {
            LOG(WARNING) << "write json to FILE* failed: write file failed";
            return false;
        }
        return true;
    }
    ...
    bool save(std::stringstream& ss, bool pretty = true) const {
        StringType str;
        if(!save(str, pretty)) {
            LOG(WARNING) << "write json failed.";
            return false;
        }
        ss << str;
        return true;
    ...
    @return boolean whether load json is sucessed.
    */
    ...
    bool load(FILE* file) {
        if (file == nullptr) {
            LOG(WARNING) << "loading file error, FILE* is nullptr";
            return false;
        }
        FileLineReader reader;
        std::string str = "";

        while (reader.getline(file, false)) {
            str.append(reader.buffer(), reader.size());
        }
    ...
    */
    ...
    {
        return m_type != value_t::error_type;
    }
  ...
  public:
    ...
    */
    ...
    template<typename ValueType>
    bool try_as(ValueType& t) const {
        if(is_null()) {
            LOG(WARNING) << "try cast empty JSON";
            return false;
        }
        try {
            t = get<ValueType>();
...
*/
    ...
    `"cannot use inner_at() with string"`
    ...
    */
  private:
    reference inner_at(size_type idx)
    {
        ...
        {
            throw std::domain_error("cannot use inner_at() with " + type_name());
            // return error_node();
        }
    }
    ...
    */
  public:
    const_reference inner_at(size_type idx) const
    {
        // at only works for arrays
        if (is_array())
        {
    ...
    `"cannot use inner_at() with boolean"`
    ...
    */
  private:
    reference inner_at(const typename object_t::key_type& key)
    {
    ...
    */
  public:
    const_reference inner_at(const typename object_t::key_type& key) const
    {
        // at only works for objects
        if (is_object())
        {
    ...
    /*!
     * \brief pico recursive at
     */
  public:
    StringType generate_pico_json_path() const {
        return "";
    }

    template<typename KeyType, class... ARGS>
    StringType generate_pico_json_path(const KeyType& key, ARGS... args) const {
        StringType ret = generate_pico_json_path(args...);
        std::stringstream ss;
        ss << key;
        return StringType("/") + ss.str() + ret;
    }

    template<typename KeyType, class... ARGS>
    reference at(const KeyType& key, ARGS... args)
    {
        if (is_structured())
        {
            StringType path;
        ...
        {
            LOG(WARNING) << "current node is not structured.";
            return s_error_node();
        }
    }

    template<typename KeyType, class... ARGS>
    const_reference at(const KeyType& key, ARGS... args) const
    {
        if (is_structured())
        {
            StringType path;
  ...
  private:
    ...
    */
    ...
    @sa @ref inner_at(const typename object_t::key_type&) for access by reference
    with range checking
    @sa @ref value() for access by value with a default value
  ...
  private:
    ...
    @note Unlike @ref inner_at(const typename object_t::key_type&), this function
    ...
    */
    ...
    @note Unlike @ref inner_at(const json_pointer&), this function does not throw
    ...
    */
  public:
    bool erase(const typename object_t::key_type& key)
    {
        ...
        if (is_object()) {
            ...
            LOG(WARNING) << "cannot use erase() with " + type_name();
            return false;
        }
    }
    ...
    @since version 1.0.0
    */
    bool erase(const size_type idx)
    {
        // this erase only works for arrays
        if (is_array()) {
            if (idx >= size()) {
                LOG(WARNING) << "array index " << std::to_string(idx) << " is out of range";
                return false;
            }
            try {
     ...
     */
    /*
    template<typename ...ARGS, typename ValueType, typename=void>
    bool modify(ARGS... args, ValueType value) {
        StringType path;
        try {
  ...
  private:
    template<typename ValueType>
    void inner_modify(const StringType& path, const ValueType& value)
    {
        try {
            *this = value;
        } catch(std::exception e) {
    ...
    template<typename KeyType, typename ValueType, class... ARGS>
    void inner_modify(const StringType& path, const KeyType& key, const ValueType& value, ARGS... args)
    {
        std::stringstream ss;
        ss << path << '[' << key << ']';
        basic_json* tmp;
  ...
  public:
    template<typename KeyType, typename ValueType, class... ARGS>
    bool modify(const KeyType& key, const ValueType& value, ARGS... args)
    {
        try {
    ...
    */
     ...
     */
    template<typename KeyType, class... ARGS>
    bool has(const KeyType& key, ARGS... args) const
    {
        basic_json ret = at(key, args...);
        return ret.is_valid();
    }
/*
    bool has(const size_type& idx) const
    {
        if (is_array()) {
            return 0 <= idx && idx < size();
        }
        LOG(WARNING) << "use size_type argument has with not array instance.";
        return false;
    }

    bool has(const StringType& key) const
    {
        if (is_object()) {
            return find(key) != end();
        }
        LOG(WARNING) << "use string argument has with not object instance.";
        return false;
    }
*/
    /*!
  ...
  public:
    ...
    @since version 1.0.0
    */
    bool push_back(basic_json&& val)
    {
        // push_back only works for null objects or arrays
        if (not(is_null() or is_array()))
        {
            LOG(WARNING) << "cannot use push_back() with " + type_name();
            return false;
        }
  ...
  private:
    ...
    */
  public:
    bool push_back(const basic_json& val)
    {
        // push_back only works for null objects or arrays
        if (not(is_null() or is_array()))
        {
    ...
    /*!
     * \brief pico add, add object to object.
     */
    ...
    bool add(const KeyType& key, const ValueType& value)
    {
        ...
        {
            LOG(WARNING) << "cannot use add() with " + type_name();
            return false;
        }
...
/*
        if (not(std::is_same<KeyType, std::string>::value or
                std::is_same<KeyType, char*>::value or
                std::is_same<KeyType, const char*>::value))
        {
            LOG(WARNING) << "key type error in add().";
            return false;
        }
        */
        if (find(key) != end())
  ...
  private:
    ...
    @note This function is required to resolve an ambiguous overload error,
          ...
          https://github.com/paradigm4::pico/json/issues/235 for more information.
  ...
  private:
    ...
    /// father node of current PicoJsonNode
    basic_json* father_node = nullptr;

  private:
    const_reference error_node() const {
        const static basic_json _error_node(value_t::error_type);
        return _error_node;
    }

    static reference s_error_node() {
        static basic_json ret;
        ret = basic_json(value_t::error_type);
        return ret;
    }
  ...
  private:
    ...
    */
    ...
    https://github.com/paradigm4::pico/json/pull/105.
    */
      ...
      public:
        ...
        */
        reference operator*() const
        {
            ...
            switch (m_object->m_type)
            {
                ...
                case basic_json::value_t::null:
                {
                    SLOG(FATAL) << "cannot get value from null type";
                    return s_error_node();
                }
                ...
                default:
                {
                    ...
                    {
                        SLOG(FATAL) << "cannot get value, out of range.";
                        return s_error_node();
                    }
                }
            }
        ...
        */
        typename object_t::key_type key() const
        {
            ...
            {
                LOG(WARNING) << "cannot use key() for non-object iterators.";
                return "";
            }
        }
...
basic_json_parser_63:
  ...
  public:
            ...
            for (const auto& reference_token : reference_tokens)
            {
                switch (ptr->m_type)
                {
                    case value_t::object:
                    {
                        ...
                        ptr = &ptr->inner_at(reference_token);
                        break;
                    }
    ...
    */
    ...
    @since version 2.0.0
    */
    reference inner_at(const json_pointer& ptr)
    {
        return ptr.get_checked(this);
    }
    ...
    @since version 2.0.0
    */
    const_reference inner_at(const json_pointer& ptr) const
    {
        return ptr.get_checked(this);
    }
    ...
    basic_json patch(const basic_json& json_patch) const
    {
        ...
        const auto operation_add = [&result](json_pointer & ptr, basic_json val)
        {
            ...
            else
            {
                ...
                if (top_pointer != ptr)
                {
                    basic_json& x = result.inner_at(top_pointer);
                }
        ...
        const auto operation_remove = [&result](json_pointer & ptr)
        {
            // get reference to parent of JSON pointer ptr
            const auto last_path = ptr.pop_back();
            basic_json& parent = result.inner_at(ptr);
        ...
        for (const auto& val : json_patch)
        {
            ...
            switch (get_op(op))
            {
                ...
                {
                    // the "path" location must exist - use inner_at()
                    result.inner_at(ptr) = get_value("replace", "value", false);
                    break;
                }
                ...
                case patch_operations::move:
                {
                    ...
                    // the "from" location must exist - use inner_at()
                    basic_json v = result.inner_at(from_ptr);
                ...
                case patch_operations::copy:
                {
                    ...
                    // the "from" location must exist - use inner_at()
                    result[ptr] = result.inner_at(from_ptr);
                    break;
                }
                ...
                case patch_operations::test:
                {
                    ...
                    try
                    {
                        ...
                        // the "path" location must exist - use inner_at()
                        success = (result.inner_at(ptr) == get_value("test", "value", false));
                    }
                    catch (std::out_of_range&)
                    {
